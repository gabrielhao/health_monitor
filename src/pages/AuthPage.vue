    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-secondary-50 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 px-8 py-6">
                <button type="button" @click="signIn()" :disabled="loading" class="btn-primary w-full">
                ...
                <div class="mt-6">
                    ...
                    <button @click="toggleMode" class="mt-4 w-full text-center text-sm text-primary-600 hover:text-primary-500 transition-colors duration-200">
                        {{ isSignUp ? 'Sign in instead' : 'Create an account' }}
                    </button>
                </div>

                <div v-if="!isSignUp" class="mt-4 text-center">
                    <button @click="showResetPassword = true" class="text-sm text-primary-600 hover:text-primary-500 transition-colors duration-200">
        ...
        <div v-if="showResetPassword" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl shadow-lg max-w-md w-full mx-4 p-6">
                ...
                <form @submit.prevent="handleResetPassword">
                    <div class="mb-4">
                        <label for="resetEmail" class="block text-sm font-medium text-neutral-700"> Email address </label>
                        <input id="resetEmail" v-model="resetEmail" type="email" required class="input-field mt-1" placeholder="Enter your email" />
                    </div>
                    ...
                    <div class="flex space-x-3">
                        <button type="submit" :disabled="loading" class="btn-primary flex-1"> Send Reset Link </button>
                        <button type="button" @click="closeResetModal" class="btn-outline flex-1"> Cancel </button>
                    </div>
                </form>
            </div>
        </div>
