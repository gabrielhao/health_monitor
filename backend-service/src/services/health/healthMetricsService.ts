...
class HealthMetricsService {
  ...
  async createHealthMetric(data: HealthMetricsCreateRequest): Promise<HealthMetric> {
    try {
      const metricData = {
        ...data,
      ...
      return await azureCosmosService.createHealthMetric(metricData)
    } catch (error) {
  ...
  async createHealthMetricsBatch(metrics: HealthMetricsCreateRequest[]): Promise<HealthMetric[]> {
    try {
      const metricsData = metrics.map(metric => ({
        ...metric,
        timestamp: metric.timestamp || new Date(),
        _partitionKey: metric.userId
      }))
      
      return await azureCosmosService.createHealthMetricsBatch(metricsData)
    } catch (error) {
      console.error('Error creating health metrics batch:', error)
      throw new Error('Failed to create health metrics batch')
    }
  }
  ...
  async deleteHealthMetrics(userId: string, metricType?: string): Promise<void> {
    try {
      ...
      throw new Error('Failed to delete health metrics')
    }
  }
  ...
  async getMetricsAggregated(
    ...
    try {
      ...
      const values = metrics.map(metric => typeof metric.value === 'number' ? metric.value : parseFloat(metric.value.toString())).filter(val => !isNaN(val))
